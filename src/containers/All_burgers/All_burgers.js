...
class All_burgers extends Component {
  ...
  allItems = () => {
    return this.props.all_burgers.map((itm, index) => (
      <Burger
        key={itm.id}
        {...itm}
        ...
        index={index}
        id={itm.id}
        enableRemoveBtn={this.enableRemoveBtn}
  ...
  render() {
    ...
    console.log(this.props.all_burgers);
    return loading ? (
      <Loader />
    ) : (
      <div style={{ padding: "7px" }}>
        {this.state.selectAllCheckBox && this.props.all_burgers.length > 0 ? (
          <button
        ...
        {this.state.enableRmv && !this.state.selectAllCheckBox ? (
          <button
            style={{
              background: "#ff8162",
              color: "white",
            ...
            onClick={this.removeSelectedItm}
          >
            Remove
          </button>
        ) : null}
